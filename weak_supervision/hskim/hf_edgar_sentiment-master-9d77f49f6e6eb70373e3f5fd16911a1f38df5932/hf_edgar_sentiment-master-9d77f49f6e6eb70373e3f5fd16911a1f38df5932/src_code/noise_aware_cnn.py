...
def train(model,train_loader , epochs = 10, lr = 0.01, batch_size = 100) :
    ...
    for epoch in range(1,epochs+1) :
        ...
        for local_batch, local_labels in train_loader:
            ...
            train_sigmoid = model(local_batch)

            train_predict = train_sigmoid > 0.5
            discrete_y = (local_labels > 0.5).unsqueeze(1)

            n_correct += (train_predict == discrete_y).sum().item()
            loss = criterion(train_sigmoid.float(),local_labels.unsqueeze(1).float())
...
def test(model,test_loader) :
    ...
    for idx,(local_batch, _) in enumerate(test_loader):
        ...
        test_sigmoid = model(local_batch)
        test_sigmoid = test_sigmoid.data.cpu()
        concat_tensor = torch.cat([concat_tensor,test_sigmoid])

    return concat_tensor

def cpu_test(model,test_X) :
    model.eval()
    output = torch.tensor([])
    for start_idx in range(0,len(test_X),10) :
        end_idx = start_idx + 10
        if end_idx > len(test_X) :
            end_idx = len(test_X)

        tmp = model(test_X[start_idx:end_idx])
        tmp = tmp.data.cpu()
